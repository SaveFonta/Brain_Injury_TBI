---
title: "First Cleaning and Exploration of Vitals and Laboratory Values"
...
The units look nice and standardized. We have an issue though: Many values are duplicated: Some patients received multiple measurements of vital parameters at entry, all with the same timestamp. I suggest we only keep one measurement each. In line with the research question, it probably makes sense to keep the measurement with the 'less severe' value, that is,

* Minimum heart rate (high heart rate indicates blood loss).
* Maximum of mean arterial pressure (low blood pressure indicates blood loss).
* Minimum temperature (fever indicates 'systemic inflammation', meaning that the body is undergoing a severe reaction to the injury).
* Maximum GCS (low GCS means more severe injury). Careful: It may happen that two measurement instances of GCS differ in the subscales while yielding the same total GCS. As advised by the client, we'd better ignore the subscales anyway.

We may have to discuss these assumptions with the supervisor/client!
...
## GCS
gcs[, Messung_Wert := as.numeric(Messung_Wert)]
gcs <- unique(gcs[!is.na(Messung_Wert)])
gcs <- gcs[, .SD[Messung_Wert == max(Messung_Wert)],
           by = .(research_case_id)]
...
# keep maximum value for each case
bp <- bp[, .SD[map == max(map)],
         by = .(research_case_id)]

## temperature
temperature[, Messung_Wert := as.numeric(Messung_Wert)]
temperature <- unique(temperature[!is.na(Messung_Wert)])
temperature <- temperature[, .SD[Messung_Wert == min(Messung_Wert)],
                           by = .(research_case_id)]

## HR (heart rate)
hr[, Messung_Wert := as.numeric(Messung_Wert)]
hr <- unique(hr[!is.na(Messung_Wert)])
hr <- hr[, .SD[Messung_Wert == min(Messung_Wert)],
         by = .(research_case_id)]
...
lactate_all <- lactate_all[, .SD[ObservationDtTm == min(ObservationDtTm)],
                           by = .(research_case_id)]
# in case of duplicates, keep the minimum value
lactate_all <- lactate_all[, .SD[Observation_value_num == min(Observation_value_num)],
                           by = .(research_case_id)]
```
...
Make a data set with one observation per patient. Following the approach also used for the vitals, we may keep the least pathological value per test.

```{r labclean}
# keep maximum value for hemoglobin, Quick
hemoglobin <- hemoglobin[!is.na(Observation_value_num)]
hemoglobin <- hemoglobin[, .SD[Observation_value_num == max(Observation_value_num)], 
                         by = .(research_case_id)]
quick <- quick[!is.na(Observation_value_num)]
quick <- quick[, .SD[Observation_value_num == max(Observation_value_num)], 
               by = .(research_case_id)]

# keep minimum value for INR, CK, lactate (which we did above)
inr <- inr[!is.na(Observation_value_num)]
inr <- inr[, .SD[Observation_value_num == min(Observation_value_num)], 
           by = .(research_case_id)]
ck <- ck[!is.na(Observation_value_num)]
ck <- ck[, .SD[Observation_value_num == min(Observation_value_num)], 
         by = .(research_case_id)]
...
* __icu_days__: numeric, if applicable ($\texttt{icu == 1}$), duration of ICU stay in days.
* __ventilation__: numeric, 0-1 indicator for mechanical ventilation.
* __ventilation_hours__: numeric, if applicable ($\texttt{ventilation == 1}$), duration of mechanical ventilation in hours.
* __death__: numeric, 0-1 indicator for death.
* __death_days__: numeric, if applicable ($\texttt{death == 1}$), length of hospital stay in days until death.
