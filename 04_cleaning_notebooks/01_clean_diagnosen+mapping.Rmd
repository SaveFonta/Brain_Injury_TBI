---
title: "Cleaning and Exploration of Diagnosen and mapping Datasets"
author: "Saverio Fontana"
date: "2025-03-05"
output: 
  html_document:
    toc: true               # Add table of contents
    toc_float: true        # Allow floating TOC
...
We begin by importing two potential data sources for diagnosis information.
...
The other sheets in the Excel seems more like summary and counting table (e.g. Pivot Table) than useful data, so we will stick for the analysis w/ the first sheet.
...
#same number of rows, but different columns 
...
We can then conclude that it is the same dataframe. For this reason we will work with Diagnosen2 (the Excel file), which has more variables. 
...
```{r}
#Drop variable name (we use display_name)
...
* **Date**: Date, the date when the injury or event occurred (format: YYYY-MM-DD).
