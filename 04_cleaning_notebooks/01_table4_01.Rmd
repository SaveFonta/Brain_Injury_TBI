...
# remove unnecessary cols, rename, TAKE OUT ROWS WITH NO GCS
...
head(check)
```
No obvious pattern or indications as to why they are missing in `vitals`, i.e. have missing GCS scores. As stated above, I suggested we treat them as missing until we figure out how to handle all the other missing GCS.
